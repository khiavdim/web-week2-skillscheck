...
// Create a function called 'bigOrSmall' that takes in one parameter, arr. Create a new array inside of bigOrSmall called
// 'answers'. Loop over the passed in arr param. If the number is GREATER than 100, push 'big' as a string to the answers array.
...
// Using a for loop, loop over the passed in array in reverse and add each item to the new reversed array. Finally, return the new reversed
// array
...
// Code Here
const total = myNumbers.reduce((acc, curr) => {return acc += curr})
...
myNumbers.forEach(function(item, index, array){myNumbersIndex.push(index)})
...
// Code Here
const forTheLoveOfGeorge = notGeorge.map(item => item.name = 'George')
...
// Code Here
const enemies = people.filter((item, index, array) => {return !item.friend})

//////////////////PROBLEM 20////////////////////

// Using the people array above, let's get a total of the awesomeLevel from all the people. Call the new array 'totallyAwesome'. 
// Use .reduce()

// Code Here
const totallyAwesome = people.reduce(function(prev, curr) {
	return prev + curr.awesomeLevel}, 0)
